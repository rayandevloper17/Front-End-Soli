...
const PrinterModal = ({ isOpen, onClose, productToPrint, supplername }) => {
  ...
  const handlePrintClick = async () => {
    try {
      ...
      // Header styling
      doc.setFillColor(200, 200, 200); // Set gray background color (RGB: 200, 200, 200)
      const textX = 129;
      const textY = 18;
      const padding = 7;

      // Measure text width for dynamic rectangle width
      doc.setFont("helvetica");
      doc.setFontSize(24);
      const textWidth = doc.getTextWidth("Bon De Réception");
      doc.rect(textX - padding, textY - 8, textWidth + padding * 2, 12 + padding, "F");
      doc.setTextColor(0, 0, 0); // Set text color to black
      doc.text("Bon De Réception", textX, textY, { align: "left" });


      // Client information styling
      doc.setFontSize(12);
      doc.text(`Client : ${responsesuppler.data.FullName || "Inconnu"}`, 20, 20);
      doc.text(
        `Date de commande: ${invoiceData[0]?.Dateorder
          ? new Date(invoiceData[0].Dateorder).toLocaleDateString() +
          " " +
          new Date(invoiceData[0].Dateorder).toLocaleTimeString()
          : "Inconnu"
        }`,
        20,
        30
      ...
      // Prepare table body by iterating through products
      const tableBody = invoiceData.map((item) => [
        item.Codebaar || "aucune valeur",
        item.Product || "aucune valeur",
        `${item.Quantity || 0}×(${item.Quantitypiece || 0})`,
        formatNumber(item.Pricepay),
        formatNumber(item.GeneralTotal),
      ]);
      ...
      const totalProducts = invoiceData.length;
      doc.text(`Nombre des produits: ${totalProducts}`, 20, finalY);

      // Calculate grand total
      const grandTotal = invoiceData.reduce(
        (sum, item) => sum + (Number(item.GeneralTotal) || 0),
        0
      );
      ...
      doc.text(formatNumber(grandTotal), rightColumnX + 55, finalY, {
        align: "right",
      });

      doc.text("Payé:", rightColumnX, finalY + 10);
      doc.text(formatNumber(invoiceData[0]?.MontantPay || 0), rightColumnX + 55, finalY + 10, {
        align: "right",
      });

      doc.text("Dettes :", rightColumnX, finalY + 20);
      doc.text(formatNumber(invoiceData[0]?.current_debt || 0), rightColumnX + 55, finalY + 20, {
        align: "right",
      });

      doc.text("Ancienne dettes :", rightColumnX, finalY + 30);
      doc.text(formatNumber(invoiceData[0]?.old_debt || 0), rightColumnX + 55, finalY + 30, {
        align: "right",
      });

      doc.text("Nouveau dette :", rightColumnX, finalY + 40);
      doc.text(formatNumber(invoiceData[0]?.new_debt || 0), rightColumnX + 55, finalY + 40, {
        align: "right",
      });
